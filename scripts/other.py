...
# --- 1. EXCEL FORMATTER CLASS (Handles all styling) ---
class ExcelFormatter:
    """Helper class to handle Excel styling and writing."""
    ...
    def set_column_widths(self, mode='BILLING'):
        """Sets column widths based on the mode (BILLING or OPS)."""
        if mode == 'BILLING':
    ...
    def write_data(self, mode='BILLING'):
        ...
        for row_idx, row in self.df.iterrows():
            ...
            # Special Handling for Ops Spacer/Header rows
            if mode == 'OPS' and pd.isna(cell_val): # Spacer Row
                self.worksheet.set_row(excel_row, 40)
                continue
            elif mode == 'OPS' and str(cell_val) == "TRIP_ID": # Repeated Header
            ...
            for col_num, val in enumerate(row):
                ...
                # Apply Time Format for specific columns
...
def clean_dataframe(df):
    """Clean string columns and headers."""
...
def process_data(uploaded_file):
    try:
        # Load File (try default, then xlrd)
        try:
        ...
        # Basic cleanup
        df.drop(index=1, inplace=True) # Remove row 1 (often junk)
        df.dropna(how="all", inplace=True)
        df.reset_index(drop=True, inplace=True)

        # 1. Identify Trip IDs
        # Assumes Trip ID is in col 10 and starts with 'T'
        df["Trip_ID_Clean"] = df[10].astype(str).apply(lambda x: x if str(x).startswith("T") else pd.NA).ffill()

        # 2. Separate Data
        # Headers contain "UNITED FACILITIES", Passengers are numbered 1-5
        mask_headers = df[1].astype(str).str.contains("UNITED FACILITIES", na=False)
        mask_pax = df[0].astype(str).str.match(r"^[1-5]$")
        ...
        # 4. Handle Date/Time Split
        # Robust split: handles "Login 10:00", "Login", or empty
        ...
        final['Direction'] = final['Direction'].replace({'Login': 'Pickup', 'Logout': 'Drop'}, regex=True)
        ...
        # File Naming
        date_str = final['TRIP_DATE'].iloc[0] if not final.empty else "Unknown"
        dir_str = final['DIRECTION'].iloc[0].title() if 'DIRECTION' in final.columns and not final.empty else "Report"
        filename = f"{date_str} {dir_str}"
...
st.title("✈️ Air India TripSheet Cleaner")
st.markdown("Upload your raw trip sheet to generate clean **Billing** and **Operations** files.")
...
if uploaded_file:
    with st.spinner('🚀 Processing data...'):
        ...
        if billing_df is not None:
            ...
            st.divider()
            st.caption("Preview of processed data:")
